                    ...
                    <div class="widget-content widget-content-area">
                        <div class="table-responsive">
                            <table class="table table-bordered table-hover table-condensed mb-4">
                                <thead>
                                <tr>
                                    <th>نام اختصاری قطب</th>
                                    <th>قطب مثبت (سمت راست)</th>
                                    <th>قطب منفی (سمت چپ)</th>
                                    <th>آزمون مرتبط</th>
                                ...
                                @foreach($poles as $pole)
                                    <tr>
                                        ...
                                        <td>{{$pole->positive}}</td>
                                        <td>{{$pole->negative}}</td>
                                        <td>{{$pole->exam->name}}</td>
                                        <td class="text-center">
                                            <ul class="table-controls">
                                                <li><a href="{{route('pole.edit',$pole->id)}}" data-toggle="tooltip" data-placement="top" title="" data-original-title="Edit"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-check-circle text-primary"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg></a></li>
                                                <li><form action="{{route('pole.destroy',$pole->id)}}" method="POST">
                                                        @csrf
                                                        @method('DELETE')
